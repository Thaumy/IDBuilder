...
<head>
    <title>Code coverage report for vue-cli-plugin-electron-builder/lib/webpackConfig.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">vue-cli-plugin-electron-builder/lib</a> webpackConfig.js</h1>
        <div class='clearfix'>
...
async function chainWebpack (api, pluginOptions, config) {
  ...
  if (process.env.IS_ELECTRON) {
    ...
    // Set IS_ELECTRON
    <span class="missing-if-branch" title="else path not taken" >E</span>if (config.plugins.has('define')) {
      config.plugin('define').tap((args) =&gt; {
        args[0]['process.env'].IS_ELECTRON = true
        return args
      })
    } else {
<span class="cstat-no" title="statement not covered" >      config.plugin('define').use(DefinePlugin, [</span>
        {
    ...
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (process.env.NODE_ENV === 'development') {
...
<span class="cstat-no" title="statement not covered" >    config.module</span>
      .rule('shebang')
      .test(/\.js$/)
      .use('shebang')
      .loader('shebang-loader')
<span class="cstat-no" title="statement not covered" >    config.externals({</span>
  ...
  const externalsList = Object.keys(dependencies || {}).reduce(
    (depList, dep) =&gt; {
      ...
      try {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!pgkString) {
<span class="cstat-no" title="statement not covered" >          throw new Error(`Could not find a package.json for module ${dep}`)</span>
        }
...
<span class="cstat-no" title="statement not covered" >        depList.push(dep)</span>
      }
      return depList
    },
    []
